...
async def generate_answer_stream_generator(request: RagRequest, db: Session):
    ...
    session_id = request.session_id or f"rag_{request.kb_id}_{hash(request.query) % 100000}"
    user_message = None
    ai_response = []
    has_sources = False
    ...
    try:
        ...
        try:
            ...
            user_message = ConversationHistory(
                ...
                message_metadata={"kb_id": request.kb_id}
            )
            db.add(user_message)
            db.commit()
        ...
        if llm_provider and sources:
            ...
            try:
                ...
                for i in range(0, len(fallback), 20):
                    ai_response.append(fallback[i:i+20])
            ...
            ai_response.append('抱歉，我在知识库中没有找到与您问题相关的信息。')
        ...
        if full_response:
            try:
                ...
                ai_message = ConversationHistory(
                    ...
                    message_metadata={"kb_id": request.kb_id, "has_sources": has_sources}
                )
                db.add(ai_message)
                db.commit()
            except Exception as e:
